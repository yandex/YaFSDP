...
class YaFSDPParam:
    ...
    _sharded_param_grad: torch.Tensor | None
    _all_gather_input: torch.Tensor | None
    _unsharded_param: nn.Parameter
    _unsharded_param_grad: torch.Tensor | None
    ...
    def __init__(
        ...
        if self.is_dtensor:
            ...
            if len(self._tp_spec.placements) != 1:
                raise NotImplementedError(
                    f"YaFSDP only supports 1D TP, not {self._tp_spec.placements}"
                )
            split_factor = self._tp_spec.num_shards_map[0]
            assert self._spmd_mesh.ndim == 2, (  # noqa: PLR2004
                f"_spmd_mesh.ndim can only be 2 but got {self._spmd_mesh.ndim}."
            )
            self._spmd_placements: tuple[Placement, ...]
            dp_shard_tp_placement = (
                (
                    _YaFSDPStridedShard(split_factor=split_factor)
                    if split_factor > 1
                    else YaFSDPShard()
                ),
    ...
    def _init_sharded_param(
        ...
        sharded_param = self.param_data.new_zeros(
            (sharded_param_data.numel(),), dtype=sharded_param_data.dtype
        )
        if hasattr(sharded_param, "__tensor_flatten__"):
            inner_tensors = [
                getattr(sharded_param, attr_name)
                for attr_name in sharded_param.__tensor_flatten__()[0]
            ]
        else:
    ...
    def init_all_gather_output(
        ...
        unsharded_param_grad: torch.Tensor | None,
    ):
        ...
        if self.sharded_param.requires_grad:
            ...
            if self.is_dtensor:
                unsharded_param_grad = DTensor(
                    unsharded_param_grad, self._tp_spec, requires_grad=False
                )
    ...
    def init_unsharded_param(self):
        ...
        if self.is_dtensor:
            unsharded_param = DTensor(
                unsharded_param, self._tp_spec, requires_grad=False
            )
    ...
    def to_sharded(self, training_state: TrainingState | None = None) -> None:
        self._setattr_on_modules(self.sharded_param)
        self.sharded_state = ShardedState.SHARDED

    def to_unsharded(self, training_state: TrainingState | None = None) -> None:
    ...
    def register_unsharded_param_with_post_backward_hook(
        ...
        self._unsharded_params_with_post_backward_hook.append(
            cast("nn.Parameter", unsharded_param)
        )
